...
public class SimpleList {
  ...
  public static List<Boolean> applicantAcceptable(List<Integer> scores, int threshold) {
     ...
     * Use a FOR-EACH loop.
     */
      // this for loop determines if each individual score is above the thresh hold
    for (int score : scores) {
      if (score > threshold) {  // yes it is greater than the thresh hold
        highScores.add(true);  // yes it is greater so true
      }
      else {  // otherwise it is not greater, so false
