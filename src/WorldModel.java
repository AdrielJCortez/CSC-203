    ...
    */
    ...
    private void parseSapling(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.SAPLING_NUM_PROPERTIES) {
            ...
            Sapling entity = Entity.createSapling(id, pt, imageStore.getImageList(Entity.SAPLING_KEY), health);
    ...
    private void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.DUDE_NUM_PROPERTIES) {
            DudeNotFull entity = Entity.createDudeNotFull(id, pt, Double.parseDouble(properties[Entity.DUDE_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.DUDE_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Entity.DUDE_RESOURCE_LIMIT_IDX]), imageStore.getImageList(Entity.DUDE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseDoctor(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.DOCTOR_NUM_PROPERTIES) {
            Doctor entity = Entity.createDoctor(id, pt, Double.parseDouble(properties[Entity.DOCTOR_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.DOCTOR_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Entity.DOCTOR_RESOURCE_LIMIT_IDX]), imageStore.getImageList(Entity.DOCTOR_KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.DOCTOR_KEY, Entity.DOCTOR_NUM_PROPERTIES));
        }
    }

    private void parseClown(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.CLOWN_NUM_PROPERTIES) {
            Clown entity = Entity.createClown(id, pt, Double.parseDouble(properties[Entity.CLOWN_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.CLOWN_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Entity.CLOWN_RESOURCE_LIMIT_IDX]), imageStore.getImageList(Entity.CLOWN_KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.DOCTOR_KEY, Entity.DOCTOR_NUM_PROPERTIES));
        }
    ...
    private void parseFairy(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.FAIRY_NUM_PROPERTIES) {
            Fairy entity = Entity.createFairy(id, pt, Double.parseDouble(properties[Entity.FAIRY_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.FAIRY_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.FAIRY_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseTree(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.TREE_NUM_PROPERTIES) {
            Tree entity = Entity.createTree(id, pt, Double.parseDouble(properties[Entity.TREE_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.TREE_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Entity.TREE_HEALTH_IDX]), imageStore.getImageList(Entity.TREE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseObstacle(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.OBSTACLE_NUM_PROPERTIES) {
            Obstacle entity = Entity.createObstacle(id, pt, Double.parseDouble(properties[Entity.OBSTACLE_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.OBSTACLE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseFWheel(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.FWHEEL_NUM_PROPERTIES) {
            FerrisWheel entity = Entity.createFerrisWheel(id, pt, Double.parseDouble(properties[Entity.FWHEEL_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.FWHEEL_KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.FWHEEL_KEY, Entity.FWHEEL_NUM_PROPERTIES));
        }
    }

    private void parseTent(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.TENT_NUM_PROPERTIES) {
            Tent entity = Entity.createTent(id, pt, Double.parseDouble(properties[Entity.TENT_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.TENT_KEY));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.TENT_KEY, Entity.TENT_NUM_PROPERTIES));
        }
    }

    private void parseHospital(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.HOSPITAL_NUM_PROPERTIES) {
            Hospital hospital = new Hospital("hospital", pt, imageStore.getImageList("hospital"),
                    1, 1, 0.123, 0.123, 100, 100);
            this.tryAddEntity(hospital);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.HOSPITAL_KEY, Entity.HOSPITAL_NUM_PROPERTIES));
        }
    }

    private void parseTile(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.HOSPITAL_NUM_PROPERTIES) {
            Tile tile = new Tile("tile", pt, imageStore.getImageList("tile"),
                    1, 1, 0.123, 0.123, 100, 100);
            this.tryAddEntity(tile);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.HOSPITAL_KEY, Entity.HOSPITAL_NUM_PROPERTIES));
        }
    }
    ...
    private void parseHouse(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.HOUSE_NUM_PROPERTIES) {
            House entity = Entity.createHouse(id, pt, imageStore.getImageList(Entity.HOUSE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseStump(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.STUMP_NUM_PROPERTIES) {
            Stump entity = Entity.createStump(id, pt, imageStore.getImageList(Entity.STUMP_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    public void parseEntity(String line, ImageStore imageStore) {
        ...
        if (properties.length >= ENTITY_NUM_PROPERTIES) {
            ...
            switch (key) {
                ...
                case Entity.FWHEEL_KEY -> this.parseFWheel(properties, pt, id, imageStore);
                case Entity.DOCTOR_KEY -> this.parseDoctor(properties, pt, id, imageStore);
                case Entity.CLOWN_KEY -> this.parseClown(properties, pt, id, imageStore);
                case Entity.TENT_KEY -> this.parseTent(properties, pt, id, imageStore);
                case Entity.HOSPITAL_KEY -> this.parseHospital(properties, pt, id, imageStore);
                case Entity.TILE_KEY -> this.parseTile(properties, pt, id, imageStore);
